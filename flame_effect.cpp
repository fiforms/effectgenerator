...
public:
    ...
    std::vector<EffectOption> getOptions() const override {
        ...
        std::vector<Opt> opts;
        opts.push_back({"--sim-multiplier", "float", 0.25, 16.0, true, "Simulation size divisor after padding expansion (output*(1+padding)/multiplier)", "2.0", true});
        opts.push_back({"--sim-pad-left", "float", 0.0, 4.0, true, "Extra simulation width left of visible frame (in visible-frame widths)", "0.25", true});
        opts.push_back({"--sim-pad-right", "float", 0.0, 4.0, true, "Extra simulation width right of visible frame (in visible-frame widths)", "0.25", true});
        opts.push_back({"--sim-pad-top", "float", 0.0, 4.0, true, "Extra simulation height above visible frame (in visible-frame heights)", "0.25", true});
        opts.push_back({"--sim-pad-bottom", "float", 0.0, 4.0, true, "Extra simulation height below visible frame (in visible-frame heights)", "0.25", true});
        opts.push_back({"--threads", "int", 0, 128, true, "Thread count for simulation passes (0 = auto)", "0", true});
        opts.push_back({"--substeps", "int", 1, 8, true, "Simulation substeps per output frame", "2", true});
        ...
        opts.push_back({"--source-heat", "float", 0.0, 20.0, true, "Heat injection strength", "3.2", false});
        opts.push_back({"--source-smoke", "float", 0.0, 10.0, true, "Smoke injection strength", "1.1", false});
        opts.push_back({"--source-updraft", "float", 0.0, 300.0, true, "Initial upward velocity impulse", "200.0", false});
        opts.push_back({"--turbulence", "float", 0.0, 120.0, true, "Lateral jitter from source turbulence", "18.0", false});
        opts.push_back({"--wobble", "float", 0.0, 3.0, true, "Base side-to-side source wobble / airflow jitter", "0.1", true});
        opts.push_back({"--flicker", "float", 0.0, 1.5, true, "Heat flicker amount (random drop/rebuild cycles)", "0.75", false});
        opts.push_back({"--crosswind", "float", 0.0, 80.0, true, "Ambient lateral air motion strength", "6.0", false});
        opts.push_back({"--stir", "float", 0.0, 80.0, true, "Large-scale room stirring airflow strength", "0.0", true});
        opts.push_back({"--stir-scale", "float", 0.2, 6.0, true, "Spatial scale of room stirring (lower = larger eddies)", "1.8", true});
        opts.push_back({"--stir-speed", "float", 0.0, 1.0, true, "Temporal speed of room stirring field", "0.08", true});
        opts.push_back({"--stir-anisotropy", "float", 0.0, 1.0, true, "Bias stirring toward horizontal flow", "0.55", true});
        opts.push_back({"--initial-air", "float", 0.0, 80.0, true, "Initial random airflow strength", "40.0", true});
        opts.push_back({"--buoyancy", "float", 0.0, 300.0, true, "Buoyancy from temperature", "220.0", true});
        opts.push_back({"--cooling", "float", 0.0, 3.0, true, "Temperature cooling rate", "0.45", true});
        opts.push_back({"--cooling-aloft", "float", 0.0, 4.0, true, "Extra cooling toward the top of the frame", "0.5", true});
        opts.push_back({"--smoke-dissipation", "float", 0.0, 3.0, true, "Smoke dissipation rate", "0.5", true});
        opts.push_back({"--velocity-damping", "float", 0.0, 3.0, true, "Velocity damping rate", "0.10", true});
        opts.push_back({"--vorticity", "float", 0.0, 200.0, true, "Vorticity confinement strength", "75.0", true});
        opts.push_back({"--flame-intensity", "float", 0.0, 5.0, true, "Brightness of flame emission", "1.25", false});
        opts.push_back({"--smoke-intensity", "float", 0.0, 3.0, true, "Opacity of smoke", "0.92", true});
        opts.push_back({"--flame-cutoff", "float", 0.0, 1.5, true, "Heat rolloff scale for smooth flame response (lower = easier ignition)", "0.15", true});
        opts.push_back({"--flame-sharpness", "float", 0.5, 6.0, true, "Curve exponent for smooth flame response", "2.0", true});
        opts.push_back({"--smokiness", "float", 0.0, 2.0, true, "Overall amount of smoke produced and rendered", "0.85", false});
        opts.push_back({"--smoke-darkness", "float", 0.0, 1.0, true, "Smoke color from light gray (0) to near-black (1)", "0.1", false});
        opts.push_back({"--age-rate", "float", 0.0, 8.0, true, "How fast emitted flame ages as it rises", "1.6", true});
        opts.push_back({"--age-cooling", "float", 0.0, 8.0, true, "Extra cooling strength based on thermal age", "0.68", true});
        opts.push_back({"--age-power", "float", 0.5, 4.0, true, "Power curve for age-based cooling/taper", "1.5", true});
        opts.push_back({"--age-taper", "float", 0.0, 4.0, true, "How strongly thermal age suppresses visible flame", "1.1", true});
        return opts;
    }
